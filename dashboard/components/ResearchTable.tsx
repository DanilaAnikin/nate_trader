...
function ActionBadge({ action }: { action: string }) {
  const styles: Record<string, string> = {
    BUY: "bg-green/8 text-green",
    HOLD: "bg-amber/8 text-amber",
    SELL: "bg-red/8 text-red",
  };
  return (
    <span className={`inline-block px-2.5 py-1 rounded-lg text-xs font-bold tracking-wide ${styles[action] || ""}`}>
      {action}
    </span>
  );
}
  ...
  return (
    <div className="min-w-[120px]">
      <div className="flex items-center justify-between mb-1.5">
        ...
        <span className="text-xs font-semibold text-foreground tabular-nums">{value}<span className="text-muted font-normal">/{max}</span></span>
      </div>
...
function ConfidenceScore({ score }: { score: number }) {
  ...
  const bg = score >= 65 ? "bg-green/6" : score >= 40 ? "bg-amber/6" : "bg-red/6";
...
export default function ResearchTable({ symbols }: ResearchTableProps) {
  ...
  if (!symbols || Object.keys(symbols).length === 0) {
    return (
      <div className="glass-card p-8 text-center">
        ...
        <p className="text-foreground font-medium">No research data available</p>
  ...
  const filters: { key: FilterAction; label: string; count: number; activeClass: string }[] = [
    { key: "ALL", label: "All", count: entries.length, activeClass: "bg-blue/8 text-blue" },
    { key: "BUY", label: "Buy", count: entries.filter(([, d]) => d.confidence?.action === "BUY").length, activeClass: "bg-green/8 text-green" },
    { key: "HOLD", label: "Hold", count: entries.filter(([, d]) => d.confidence?.action === "HOLD").length, activeClass: "bg-amber/8 text-amber" },
    { key: "SELL", label: "Sell", count: entries.filter(([, d]) => d.confidence?.action === "SELL").length, activeClass: "bg-red/8 text-red" },
  ];
      ...
      <div className="divide-y divide-border">
        {filtered.map(([symbol, data]) => {
          ...
          return (
            <div key={symbol}>
              <div
                className="px-5 py-4 hover:bg-surface/50 cursor-pointer transition-colors"
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-4">
                    ...
                    {t.rsi_14 != null && (
                      <span className="text-xs text-muted">RSI <span className="text-foreground font-medium">{t.rsi_14.toFixed(1)}</span></span>
                    )}
              ...
              >
                <div className="px-5 py-4 bg-surface border-t border-border text-xs">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          ...
                          <span className="text-foreground font-medium">
                            ...
                            {t.above_sma20 != null && (
                              <span className={`ml-1.5 text-[10px] px-1.5 py-0.5 rounded ${t.above_sma20 ? "bg-green/8 text-green" : "bg-red/8 text-red"}`}>
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="text-foreground font-medium">{t.sma_50 ? `$${t.sma_50.toFixed(2)}` : "N/A"}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted">MACD / Signal</span>
                          <span className="text-foreground font-medium">{t.macd?.toFixed(2) ?? "N/A"} / {t.macd_signal?.toFixed(2) ?? "N/A"}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted">Volume Ratio</span>
                          <span className={`font-medium ${(t.volume_ratio ?? 0) >= 1.2 ? "text-green" : "text-foreground"}`}>
                      ...
                      {data.news_headlines && data.news_headlines.length > 0 ? (
                        <div className="space-y-1.5">
                          {data.news_headlines.slice(0, 3).map((h, i) => (
                            <p key={i} className="text-secondary leading-relaxed line-clamp-2">{h}</p>
                          ))}
                        </div>
                      ) : (
                        <p className="text-muted italic">No recent headlines</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      {data.perplexity ? (
                        <div className="space-y-2">
                          ...
                          <div className="flex items-start gap-1.5 text-red/70">
                        ...
                        <p className="text-muted italic">Not yet analyzed</p>
                      )}
                    </div>
                  </div>
                </div>
