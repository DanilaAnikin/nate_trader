    ...
    <div className="glass-card overflow-hidden">
      <div className="px-5 py-3.5 border-b border-border flex items-center gap-2">
        {tabs.map((t) => (
          ...
          >
            {t.label}
            <span className={`ml-1.5 ${tab === t.key ? "text-blue/50" : "text-muted/50"}`}>
              {t.count}
            </span>
          </button>
        ))}
...
function ScoreBar({ value, max, color }: { value: number; max: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="w-14 h-2 bg-surface rounded-full overflow-hidden">
    ...
    <table className="w-full text-sm">
      <thead>
        <tr className="text-xs text-muted uppercase tracking-wider border-b border-border">
          ...
          <th className="text-right px-4 py-3">5d Ret</th>
          <th className="text-right px-5 py-3">Vol Ratio</th>
        </tr>
      </thead>
      <tbody>
        {sorted.map(([symbol, d]) => {
          ...
          return (
            <tr key={symbol} className="border-b border-border/50 hover:bg-surface/50 transition-colors">
              <td className="px-5 py-3 font-medium text-foreground">{symbol}</td>
              <td className="px-4 py-3 text-right text-foreground">${d.technicals.price.toFixed(2)}</td>
              ...
              <td className="px-4 py-3 text-center">
                <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${actionColor}`}>
                  {d.confidence.action}
                </span>
              </td>
    ...
    <table className="w-full text-sm">
      ...
      <tbody>
        {data.map((d) => {
          ...
          return (
            <tr key={d.symbol} className="border-b border-border/50 hover:bg-surface/50 transition-colors">
              <td className="px-5 py-3 font-medium text-foreground">{d.symbol}</td>
              <td className="px-4 py-3">
                <div className="flex items-center gap-2">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 divide-x divide-border">
      <div>
        <div className="px-5 py-2.5 border-b border-border">
          <span className="text-xs text-green font-semibold uppercase tracking-wider">Gainers ({gainers.length})</span>
        </div>
        <table className="w-full text-sm">
          <tbody>
            {gainers.map((d, i) => (
              <tr key={`g-${d.symbol}-${i}`} className="border-b border-border/50 hover:bg-surface/50 transition-colors">
                <td className="px-5 py-2.5 font-medium text-foreground">{d.symbol}</td>
                <td className="px-4 py-2.5 text-right text-secondary">${d.price.toFixed(2)}</td>
                <td className="px-5 py-2.5 text-right text-green font-medium">+{d.change_pct.toFixed(2)}%</td>
              </tr>
            ))}
            {gainers.length === 0 && (
              <tr><td className="px-5 py-4 text-center text-muted" colSpan={3}>None</td></tr>
            )}
          </tbody>
        </table>
      </div>
      <div>
        <div className="px-5 py-2.5 border-b border-border">
          <span className="text-xs text-red font-semibold uppercase tracking-wider">Losers ({losers.length})</span>
        </div>
        <table className="w-full text-sm">
          <tbody>
            {losers.map((d, i) => (
              <tr key={`l-${d.symbol}-${i}`} className="border-b border-border/50 hover:bg-surface/50 transition-colors">
                <td className="px-5 py-2.5 font-medium text-foreground">{d.symbol}</td>
                <td className="px-4 py-2.5 text-right text-secondary">${d.price.toFixed(2)}</td>
                <td className="px-5 py-2.5 text-right text-red font-medium">{d.change_pct.toFixed(2)}%</td>
              </tr>
            ))}
            {losers.length === 0 && (
              <tr><td className="px-5 py-4 text-center text-muted" colSpan={3}>None</td></tr>
    ...
    <div className="p-5">
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2.5">
        {data.map((t, i) => (
          <div
            key={t}
            className="flex flex-col items-center justify-center p-3 rounded-xl bg-purple/[0.04] border border-purple/10 hover:border-purple/20 hover:bg-purple/[0.06] transition-all duration-200 cursor-default"
          >
