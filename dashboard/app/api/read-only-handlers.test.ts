 ...
 * no user intent and no audit entry. It also makes the write freeze
 * (`lib/frozen.ts` on this containment bridge) unenforceable, because a freeze
 * that blocks the mutating verbs still lets a dashboard left open keep writing.
 *
...
describe("every mutating handler is behind the write freeze", () => {
  ...
  // On this containment bridge the freeze is `frozenResponse()` from
  // lib/frozen.ts: unconditional, a property of the artifact. It is NOT
  // `maintenanceBlock()` (lib/maintenance.ts), which returns null whenever
  // DASHBOARD_MAINTENANCE_MODE is unset and is therefore an environment
  // setting, not a freeze. A handler that reached for it here would be a
  // regression to the development dashboard's weaker contract.
  it.each(MUTATING)("%s returns frozenResponse() unconditionally", (file, label) => {
    ...
    const freezes = source.match(/return frozenResponse\(\)/g) ?? [];
    expect(handlers.length, `${label} declares no mutating handler`).toBeGreaterThan(0);
    // One freeze per mutating handler: a shared import is not a control.
    expect(freezes.length, `${label} freezes ${freezes.length} of ${handlers.length}`)
      .toBeGreaterThanOrEqual(handlers.length);
    expect(source, `${label} relies on the env-gated maintenanceBlock()`)
      .not.toMatch(/maintenanceBlock\(/);
  });
});
