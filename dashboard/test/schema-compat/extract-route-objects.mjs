...
if (args.includes("--help")) {
  process.stdout.write(
    [
      ...
      "  --extra-route <file>  additionally treat <file> as a route (negative control)",
      "",
    ].join("\n"),
  );
  process.exit(0);
...
if (OUT_SQL) {
  ...
  const rows = objectList
    .map((o) => {
      ...
      return `    ('${o.object}', '${o.kind}', '${o.routes.join(" ")}')`;
    })
    .join(",\n");
  writeFileSync(
    OUT_SQL,
    `-- generated by extract-route-objects.mjs — do not edit\n` +
      `create temporary table wanted(obj_name text, obj_kind text, used_by text);\n` +
      `insert into wanted(obj_name, obj_kind, used_by) values\n${rows};\n`,
  );
}
